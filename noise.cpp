...
float noise(float xin, float yin, float zin)
{
    ...
    // c = 1/6.
    float x1 = x0 - i1 + G3; // Offsets for second corner in (x,y,z)
    // coords
    ...
    float x3 = x0 - 1.0f + 3.0f * G3; // Offsets for last corner in
    // (x,y,z) coords
...
float octaves(int octaves, float x, float y, float z, int ridged)
{
    ...
    if (ridged)
    {
        for (i = 0; i < octaves; i++)
        {
            float twopoweri = (float)(1 << i);
            value += fastabs(noise(x * twopoweri,
                             y * twopoweri,
                             z * twopoweri) / twopoweri);
        }
    }
    else
    {
...
float biLerp(float x, float y, float q11, float q12,
